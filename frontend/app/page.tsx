...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-gray-50 pb-16 md:pb-0">
          ...
          <div className="flex items-center gap-6 flex-1">
            ...
            <div className="hidden md:flex flex-1 max-w-xl relative">
              <input 
                ...
                className="w-full bg-gray-100 border-none rounded-xl py-2.5 pl-10 pr-4 focus:ring-2 focus:ring-[var(--color-accent)] outline-none"
                value={inputValue}
                onChange={(e) => {
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-40 flex">
        <button 
          onClick={() => setIsMobileSortOpen(true)}
          className="flex-1 flex items-center justify-center gap-2 py-4 font-semibold text-gray-700 active:bg-gray-50 border-r border-gray-200"
        >
        ...
        <button 
          ...
          className="flex-1 flex items-center justify-center gap-2 py-4 font-semibold text-gray-700 active:bg-gray-50 relative"
        >
          <svg className="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path></svg>
          Filter
          {activeFilterCount > 0 && (
            <span className="absolute top-2 right-10 bg-[var(--color-accent)] text-white text-[10px] font-bold w-5 h-5 flex items-center justify-center rounded-full border-2 border-white">
              {activeFilterCount}
            </span>
          )}
        </button>
